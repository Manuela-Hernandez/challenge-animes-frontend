    ...
    <section className='bg-sky-900'>
      ...
      <div >
        <div className='grid grid-cols-1 mx-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5'>
          {
            animeList.map((anime, index) => (
              <CardAnime
                ...
                message={anime.recommendationMessage}
              />
            ))
          }
        </div>
        <div className='grid grid-cols-2 mt-4 mx-5'>
          <button
            className='w-auto h-8 rounded-lg text-xl border-solid bg-slate-300 justify-self-start mb-5 px-2'
            onClick={previousPage}
            disabled={currentPage <= 1}>
            Previous
          </button>
          <button
            className='w-auto h-8 rounded-lg text-xl border-solid bg-slate-300 justify-self-end mb-5 px-2'
            onClick={nextPage}>
            Next
          </button>
        </div>
